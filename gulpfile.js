...
gulp.task('gzjs',function(){
  gulp.src('./build-place/dev/js/*.js')
  .pipe(gzip())
  .pipe(gulp.dest('./build-place/dist/js'));
})
// 压缩 js 文件
...
gulp.task('ugcss',function(){
    gulp.src('./build-place/dev/css/*.css')
        .pipe(cssUglify())
        .pipe(gulp.dest('./build-place/dist/css'))
})
 
gulp.task('ugimage',function(){
    gulp.src('./build-place/dev/image/*.*')
        .pipe(imageMin({progressive: true}))
        .pipe(gulp.dest('./build-place/dist/images'))
})
